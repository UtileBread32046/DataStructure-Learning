...
void initMatrixGraph(MatrixGraph *graph, char *names[], int nodeNum, int directed, int edgeValue) {
    graph->nodeNum = nodeNum; // 顶点数
...
void addEdgeToGraph(MatrixGraph *graph, int v1, int v2, int weight) {
    if (v1 < 0 || v2 < 0 ||  v1 > graph->nodeNum || v2 > graph->nodeNum) {
